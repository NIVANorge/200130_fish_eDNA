...
# Tm 64 degrees:
```{r}
quant_ssalar_finsval3 <- read_delim(here("data", "EEG_2023-02-03 15-10-29_CFXTOUCH_Ssalar_Tm64_GJE_Torskv.txt")) %>% 
  mutate(plate = rep("plate3", dim(.)[1])) %>% 
  dplyr::filter(Sample %in% as.character(seq(567,584,1))) %>% 
  arrange(desc(Sample)) %>%
  dplyr::mutate(tech_rep = factor(c(rep(c(1:2), 14), 1,1,2,1,2,1)))
...
quant_ssalar_finsval_join <- quant_ssalar_finsval3 %>% dplyr::select(Content, Sample, Cq, SQ, plate, tech_rep) %>%  dplyr::mutate(SQ = as.numeric(SQ)) %>% 
  dplyr::arrange(desc(Sample)) %>% dplyr::mutate(Sample = factor(Sample)) %>% 
left_join(., samples, by = c("Sample" = "provenr")) %>% mutate(SQ_vol = SQ/volum) %>% 
  mutate(stasjon = factor(as.character(stasjon), ordered = T, levels = c("4","3","2","1","5","6", "TV", "BT", "LS")))
